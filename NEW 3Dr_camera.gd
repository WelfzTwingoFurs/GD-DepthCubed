...
func _draw():
	...
	for object in inview:#what do we see?
		if object.get_type(): for poly in object.poly_faces:#get_type() == true == polygon array. How many polys in array?
			...
			if size > 0:
				drawlist.append(
				#[-zindex_max(-((Vector3(global_position.x, global_position.y, get_parent().position_z).distance_to(Vector3i(object.global_position.x, object.global_position.y, object.position_z))) * (8192/drawdist)-4096)),
				[Vector3(global_position.x, global_position.y, get_parent().position_z).distance_to(Vector3i(object.global_position.x, object.global_position.y, object.position_z)),
				sprite, (Vector2(
					tan(angle),
