...
func _draw():
	...
	for object in inview:#what do we see?
		...
		#Z sorting goes here!
		#inview.sort_custom(sort_ascending)
		
		
		#get_type() == true == polygon array. 
		if object.get_type(): for poly in object.poly_faces:#How many polys in array?
			var current =  PackedVector2Array([])
			
			# n(vertex/info), of poly(current polygon), of object.poly_faces(list in poly), of object(model in world)
			for n in poly:#How many vertexes in poly?
				
				#INT? Means it's vertex! Calculating...
				...
				elif Geometry2D.triangulate_polygon(current) && Geometry2D.is_polygon_clockwise(current):
					#print(PackedVector2Array(current))
					draw_set_transform(Vector2i(),false, Vector2(1,1))
					
					if typeof(n) == TYPE_COLOR:#draw_colored_polygon
						draw_colored_polygon(current, n)#flat polygon! 
					else:
						draw_colored_polygon(current, poly[poly.size()-1], select_uv(current.size()), load(n))#textured polygon!
		...
		############################################################################################
			...
			if size > 0:
				...
				draw_set_transform(Vector2i(),false, Vector2(size*sign(rotangle),abs(size*sign(rotangle))))
				...
				draw_texture(sprite,
					...
					(object.position_z-get_parent().position_z)/29.4  /  (global_position.distance_to(object.global_position)/8192 * cos(angle))
						...
						- Vector2(
							sprite.get_size().x/2,
							sprite.get_size().y/(2*int(!object.offset_z) + 1*int(object.offset_z))),
