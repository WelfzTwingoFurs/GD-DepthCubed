...
func _draw():
	if receive.size() > 0:
		...
		elif typeof(receive[1]) == TYPE_COLOR:#Received color, is polygon!
			draw_set_transform(Vector2i(),false, Vector2(1,1))
			if receive.size() == 2: draw_colored_polygon(receive[0], receive[1])#flat polygon! 
			else: draw_colored_polygon(receive[0], receive[1], receive[2], load(receive[3]))#textured polygon!
