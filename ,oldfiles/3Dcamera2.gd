...
func _draw():
	...
	for object in inview:
		if object.get_type():# == Polygon 
			...
			for now in object.polys:#now=pointer, in object.polys(array of arrays, list of polygons)
				for n in now:#n=pointer, in now(array, 'currently selected' polygon data)
					if typeof(n) == TYPE_INT:
						var positionX = (
						tan((Vector2(object.points[n].x, object.points[n].y) - position).angle() - midscreen)#xkusu
						 * (DisplayServer.window_get_size().x/3.5))#fov code should come here
						
						var size = pow((1-((position.distance_to(Vector2(object.points[n].x, object.points[n].y))/draw_dist) * cos((object.global_position - position).angle() - midscreen)))*2, sqrt(size_max-size_min))+size_min
			...
			#var size = (1-((position.distance_to(object.global_position)/draw_dist) * cos(xkusu)))*(size_max-size_min)+size_min
