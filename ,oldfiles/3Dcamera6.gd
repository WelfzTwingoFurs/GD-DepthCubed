...
func _draw():
	...
	var midscreenFirst = DisplayServer.window_get_size().x * (0.25 * tan(( Vector2(0,drawdist).rotated(-fov/2) ).angle() ))
	var midscreenLast  = DisplayServer.window_get_size().x * (0.25 * tan(( Vector2(0,drawdist).rotated( fov/2) ).angle() ))
		...
		if object.get_type():# == Polygon:
			...
			if size > 0:
				...
				draw_texture(object.texture, Vector2i(
					...
					)/size - object.texture.get_size()/2, object.colorR)
