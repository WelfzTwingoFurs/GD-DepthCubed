...
func _draw():
	...
	for object in inview:#what do we see?
		...
		if object.get_type(): for poly in object.poly_array:#get_type() == true == polygon array. How many polys in array?
			...
			var size = digit_to_decimal(8192-sprite.get_size().x)*object.scaleR / ((position.distance_to(object.global_position)/(DisplayServer.window_get_size().x)) * cos(angle))
			...
			if size > 0:
				...
				nodes_z[counter-1].receive = [sprite, (Vector2(
					tan(angle),
					(object.position_z-position_z)/29.4  /  (position.distance_to(object.global_position)/8192 * cos(angle))
					) * Vector2(
...
func _process(_delta):#Graphics process exclusively
	...
	$DrawContainer/Node0.receive = [$Area.rotation_degrees, #SKY & FLOOR
		[load("res://resources/sky3.png"),  3.0,( 2000-position_z)/29.4/ 4.9152 *DisplayServer.window_get_size().x/1225.0],
		[load("res://resources/sky2.png"),  2.5,( 1000-position_z)/29.4/ 4.0960 *DisplayServer.window_get_size().x/1225.0],
		[load("res://resources/sky1.png"),  2.0,( 2000-position_z)/29.4/ 3.2768 *DisplayServer.window_get_size().x/1225.0],
		[load("res://resources/sky0.png"),  1.5,( 3000-position_z)/29.4/ 1.6384 *DisplayServer.window_get_size().x/1225.0],
		[load("res://resources/cave0.png"),-1.5,( 6000-position_z)/29.4/ 4.0960 *DisplayServer.window_get_size().x/1225.0],
		[load("res://resources/cave0.png"),-1.0,( 3000-position_z)/29.4/ 3.2768 *DisplayServer.window_get_size().x/1225.0]]
...
func _physics_process(_delta):#Game process exclusively
	...
	offset.y += Input.get_axis("ply1_butt_up", "ply1_butt_down")
	$aim.position.y = offset.y
